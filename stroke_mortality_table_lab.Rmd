...
stroke_mortality <- read_csv("./stroke_mortalitybyhbr.csv")
health_boards <- read_csv("./hb14_hb19.csv")
...
```{r}
joined_data <- full_join(health_boards, stroke_mortality, by = c("HB" = "HBR"))
```
...
joined_data %>% 
  ...
  grand_summary_rows(columns = c(NumberOfDeaths, CrudeRate, EASR), 
                     ...
                     fmt = list(~ fmt_number(., decimals = 2))) %>% 
  fmt_number(columns = c(NumberOfDeaths, CrudeRate, EASR), 
             decimals = 2) %>% 
